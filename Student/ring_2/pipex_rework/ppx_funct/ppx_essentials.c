...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/05 02:00:17 by akamal-b          #+#    #+#             */
/*   Updated: 2025/02/05 02:05:25 by akamal-b         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_path(char *command, char **arg_env)
{
	...
	while (whole_path_no_colon[++row])
	{
		...
		command_to_execute = ft_strjoin(whole_path_filtered, \
											command_splitted[0]);
...
void	ppx_child(char **arg_env, char **arg_v, int *pipe_fd)
{
	...
	if (fd < 0)
		error_and_exit("Error on \"input\" file");
...
void	ppx_child2(char **arg_env, char **arg_v, int *pipe_fd)
{
	int	fd;

	fd = open(arg_v[4], O_WRONLY | O_CREAT | O_TRUNC, 0777);
	if (fd < 0)
		error_and_exit("Error on \"output\" file");
